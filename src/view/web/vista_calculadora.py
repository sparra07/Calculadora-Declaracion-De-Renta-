...
blueprint = Blueprint("vista_calculadora", __name__,"templates")
...
def crear_tablas():
    try:
        ...
        return "Tablas creadas exitosamente.<br /><a href='/'>Volver al inicio</a>"
    except Exception as e:
        return f"Error al crear las tablas: {str(e)}.<br /><a href='/'>Volver al inicio</a>"
...
@blueprint.route("/calculos")
def calculos():
    return render_template("calculos.html")

@blueprint.route("/procesar_calculo")
...
@blueprint.route("/guardar_calculo_definitivo")
def guardar_calculo_definitivo():
    try:
        ...
        # Llama a tu controlador original para insertar el cálculo
        CalculoRentaController.insertar_calculo(calculo)
        
        # Redirige y confirma mostrando el éxito asociado a la cédula del usuario
        return f"Liquidación Guardada exitosamente para el usuario con cédula: {cedula_usuario}.<br /><a href='/buscar_calculo?cedula_usuario={cedula_usuario}'>[ Ver historial de cálculos de este usuario ]</a><br /><br /><a href='/calculos'>Volver a Cálculos</a>"
...
@blueprint.route("/buscar_calculo")
def buscar_calculo():
    try:
        cedula_usuario = request.args["cedula_usuario"]
        # Llama a tu método original (en tu app.py original llamabas a buscar_calculos_por_usuario)
        lista_calculos = CalculoRentaController.buscar_calculos_por_usuario(cedula_usuario)
        
        if not lista_calculos or len(lista_calculos) == 0:
            return "No se encontró ningún cálculo para esa cédula.<br /><a href='/calculos'>Volver a Cálculos</a>"
        
        # Te lleva a ver_calculos.html pasando el historial encontrado
        return render_template("ver_calculos.html", calculos=lista_calculos)
    except Exception as e:
        return f"No se encontró ningún cálculo para esa cédula. Detalle: {str(e)}.<br /><a href='/calculos'>Volver a Cálculos</a>"

@blueprint.route("/modificar_calculo")
def modificar_calculo():
    try:
        ...
        if not calculo:
            return "No se encontró ningún cálculo con ese ID.<br /><a href='/calculos'>Volver a Cálculos</a>"
            
        return render_template("modificar_calculo.html", calculo=calculo)
    except Exception as e:
        return f"No se encontró ningún cálculo con ese ID. Detalle: {str(e)}.<br /><a href='/calculos'>Volver a Cálculos</a>"

@blueprint.route("/actualizar_calculo")
def actualizar_calculo():
    try:
        ...
        # Llama a tu método original del controlador
        CalculoRentaController.actualizar_calculo(calculo)
        return f"Cálculo modificado exitosamente para la cédula {cedula_usuario}. <a href='/buscar_calculo?cedula_usuario={cedula_usuario}'>Regresar al historial</a>"
    except Exception as e:
        return f"Error al actualizar el cálculo: {str(e)}.<br /><a href='/'>Volver al inicio</a>"

@blueprint.route("/eliminar_calculo")
@blueprint.route("/eliminar_calculo")
...
def eliminar_calculo():
    try:
        ...
        return f"Historial de cálculos eliminado exitosamente para el usuario con cédula: {cedula_usuario}. <br /><a href='/'>Volver al inicio</a>"
    except Exception as e:
